## VRP has main function to create data for training and evaluating
...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, input_size, max_load=20, max_demand=9,
                 locations=None, demands=None, seed=None):
        super(VehicleRoutingDataset, self).__init__()

        if num_samples == 1 and (locations is not None and demands is not None):
            self.static = torch.tensor(locations).unsqueeze(0)
            self.dynamic = torch.tensor(demands).unsqueeze(0)
            self.num_samples = 1
        else:
...
def render(static, tour_indices, save_path):
    """Plots the found solution with the route."""
    ...
    for i, ax in enumerate(axes):
        ...
        for j in range(len(where) - 1):
            ...
            ax.plot(x[low: high + 1], y[low: high + 1], zorder=1)

            # Print route
            route = idx[low: high + 1]
            route_coordinates = data[:, low: high + 1]
            if isVisited[i]==False:
              print("Route {}: {}".format(i, route_coordinates.T))
              isVisited[i]=True
            elif not np.array_equal(route_coordinates.T[-1], route_coordinates.T[-2]):
              print(route_coordinates.T[1:])
            elif np.array_equal(route_coordinates.T[-1], route_coordinates.T[-2]):
              print(route_coordinates.T[1:-1])
